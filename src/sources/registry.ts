...
import { RUBY_CITY, RubyCitySource } from './rubyCity/index.js';
...
export const SOURCES: readonly SourceInfo[] = [
  ...
  {
    id: MISSION_MARQUEE.id,
    label: MISSION_MARQUEE.label,
    needsBrowser: false,
  ...
  { id: RUBY_CITY.id, label: RUBY_CITY.label, needsBrowser: false, enabledByDefault: true },
  {
...
export const DEFAULT_SOURCE_IDS: readonly string[] = SOURCES.filter((s) => s.enabledByDefault).map(
  (s) => s.id,
);
...
export function buildSources(
  ...
  if (wanted.has(RUBY_CITY.id)) sources.push(new RubyCitySource());
